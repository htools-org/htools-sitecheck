...
recursiveResolver.hints.addServer(".", "127.0.0.1");
...
class Validator {
  ...
  async fetchPrereqs() {
    ...
    try {
      ...
      this.latestUpdateHeight = latestUpdate.height;
  ...
  async ensureTreeUpdated() {
    ...
    const blocksSinceLastTreeUpdate =
      this.currentHeight - (this.latestUpdateHeight % 36);
    if (blocksSinceLastTreeUpdate > 36 + 12) {
      return {
        ok: true,
        title: "Urkel tree is synced and updated",
        desc: desc,
        current: `Latest records on chain are being served to resolvers for the past ${blocksSinceLastTreeUpdate} blocks.`,
      };
    }

    if (blocksSinceLastTreeUpdate > 36) {
      const blocksTillNextUpdateSPV = 12 - (blocksSinceLastTreeUpdate - 36);
      return {
        ok: false,
        ...
        solution: `Wait for another ~${
          blocksTillNextUpdateSPV * 10
    ...
    return {
      ...
      desc: desc,
      current: `The last tree update was ${blocksSinceLastTreeUpdate} blocks ago.`,
      solution: `Wait for another ~${
        (36 - blocksSinceLastTreeUpdate) * 10
      } minutes and try again.`,
      refUrl: "https://hsd-dev.org/protocol/summary.html",
    };
  }
  ...
  async ensureCorrectTlsa() {
    ...
    const expectedTlsa = this.tlsaRecord;

    const tlsaMatched = dane.verify(
      certificate.raw,
      expectedTlsa.selector,
      expectedTlsa.matchingType,
      expectedTlsa.certificate
    );
    ...
    if (tlsaMatched) {
      ...
      return {
        ...
        desc: desc,
        current:
          this.tlsaRecord.toString() +
          "\nDoes not match the certificate used by the web server.",
