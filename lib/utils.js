...
const makeRPCRequest = async (method, params = undefined) =>
  makeRequest(
    process.env.HSD_RPC_URL,
    {
      method: 'POST',
      auth: `x:${process.env.HSD_API_KEY}`,
      body: JSON.stringify({
        method,
        params,
      }),
...
async function getLatestUpdate(tld) {
  const data = await makeRPCRequest('getnameinfo', [tld]);
  const latestData = data.result.info;
...
async function getCurrentHeight() {
  const data = await makeRPCRequest('getblockchaininfo');
